...
const InputField = ({type, placeholder, name, handleChange, address}) => (
    <input 
        ...
        name={name}
        value={address[name] || ''} // Added fallback for undefined values
        required
    />
)
...
const AddAddress = () => {
    ...
    // ✅ FIXED: Changed zipCode to zipcode to match schema
    const [address, setAddress] = useState({
        ...
        state: '',
        country: '',
        zipcode: '', // Match schema field name
        phone: '',
    });
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            // ✅ FIXED: Send address and userId separately as expected by backend
    ...
    // ✅ FIXED: Added dependencies and better user check
    useEffect(() => {
        if (!user) {
            navigate('/cart')
        }
